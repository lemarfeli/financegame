...
import { generateRandomName } from 'src/player/utils/name-generator';
import { BotManagerService } from 'src/bot-strategy/bot-manager.service.ts';
...
export class GameSessionService {
  ...
  async createSession() {
    ...
    const player = await this.playerService.createPlayer(generateRandomName(), gameSession.id, 0, true, false);    
...
async getGameResults(sessionId: number) {
  ...
  return {
    ...
    ranking: finalPlayers.map(p => ({ name: p.playerName, balance: p.playerBalance})),
  };
}
...
async getRemainingTime(sessionId: number) {
  ...
  if (!session || !session.startTime || session.gameTime === null || !session.gameStatus) {
    return { remainingTime: 0, message: 'Игра не активна или данные отсутствуют.' };
  }
