...
export class PlayerResourcesService {
  ...
  async updateSystemMarketResources() {
    await this.prisma.marketResource.deleteMany({ where: { isSystem: true } });
  ...
  async sellToMarket(playerId: number, resourceId: number) {
    ...
    await this.addPlayerResourceToMarket(resourceId);
  
    return { message: 'Ресурс продан' };
  }
  
  async addPlayerResourceToMarket(resourceId: number) {
    const existing = await this.prisma.marketResource.findFirst({
      where: { resourceId, isSystem: false },
    });
  ...
  async buyFromMarket(playerId: number, resourceId: number) {
    const resource = await this.prisma.resource.findUnique({ where: { id: resourceId } });
    if (!resource) throw new NotFoundException('Ресурс не найден');
    ...
    if (marketEntry.quantity <= 1) {
      await this.prisma.marketResource.delete({ where: { id: marketEntry.id } });
    } else {
