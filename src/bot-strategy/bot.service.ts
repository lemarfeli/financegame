...
const botStrategies = new Map<number, { strategy: Strategy, stepsUntilReview: number }>();


function getSmartStrategy(balance: number): Strategy {
  const strategies = [
...
export class BotService {
  private botTargets = new Map<number, { targetCompanyTypeId: number | null, targetTurns: number }>();
  ...
  async makeBotDecision(playerId: number) {
    ...
    if (!cached || cached.stepsUntilReview <= 0) {
      ...
      botStrategies.set(playerId, { strategy: newStrategy, stepsUntilReview: 3 + Math.floor(Math.random() * 3) });
      this.logger.log(`Бот ${playerId} переключился на стратегию ${newStrategy}`);
    } else {
    ...
    for (const task of tasks) {
      if (Math.random() < 0.8) {
        try {
          ...
          this.logger.error(`Ошибка выполнения задачи ботом ${bot.id}: ${e.message}`);
        }
      }
    }
  }
  ...
  private async handleLoans(bot: any) {
    ...
    if (loan) {
      const shouldRepay = loan.fine > 0 && bot.playerBalance >= loan.debt + loan.fine;
      if (shouldRepay) {
        try {
          await this.loanService.repayLoan(bot.id);
          this.logger.log(`Бот ${bot.id} погасил кредит`);
        } catch (e) {
          this.logger.warn(`Бот ${bot.id} не смог погасить кредит: ${e.message}`);
        }
        return;
      }
    }
    ...
    if (shouldTakeLoan) {
      const amount = 2500;
      await this.loanService.takeLoan(bot.id, amount, this.LOAN_PERIOD, this.LOAN_INTEREST_RATE);
      this.logger.log(`Бот ${bot.id} взял кредит на ${amount}`);
    }
  }

  private async handleDeposits(bot: any) {
    const activeDepositsCount = bot.deposits.filter(d => d.datePayout === null).length;
    
    if (activeDepositsCount >= 3) return;

    const totalAssets = bot.playerBalance + bot.deposits.reduce((sum, d) => sum + d.amount, 0) + bot.shares.reduce((sum, s) => sum + (s.costShares || 0), 0);
    ...
    if (amount > 500) {
      await this.depositService.createDeposit(bot.id, amount, this.DEPOSIT_PERIOD, this.DEPOSIT_PERCENTAGE);
  ...
  private async repairBrokenCompanies(bot: any) {
    ...
    for (const company of brokenCompanies) {
      try {
        ...
        this.logger.error(`Бот ${bot.id} не смог починить предприятие ${error.message}`);
      }
    }
  }

  private async openProfitableCompanies(bot: any) {
    const botState = this.botTargets.get(bot.id) || { targetCompanyTypeId: null, targetTurns: 0 };
    ...
    if (botState.targetCompanyTypeId !== null) {
      ...
      const hasAllResources = companyType.requirements.every((req) => {
        const resource = bot.resources.find((r) => r.resourceId === req.resourceId);
      ...
      if (hasAllResources && canAfford) {
        ...
        this.botTargets.set(bot.id, { targetCompanyTypeId: null, targetTurns: 0 });
        return;
      }
    ...
    for (const news of positiveNews) {
      ...
      if (!hasCompanyOfType) {
        ...
        if (companiesOfType < 3) {
          ...
          this.logger.log(`Бот ${bot.id} выбрал цель: предприятие ${news.companyTypeId}`);
          break;
        }
      }
    }
  ...
  private async tryOpenCompany(bot: any, companyTypeId: number) {
    ...
    if (missingResources.length === 0) {
      try {
        ...
        await this.companyService.createCompany(bot.id, companyTypeId, companyName);
        this.logger.log(`Бот ${bot.id} открыл предприятие ${companyTypeId}`);
      } catch (error) {
        this.logger.error(`Бот ${bot.id} не смог открыть предприятие ${error.message}`);
      }
    }
  }
  ...
  private async buyNeededResources(bot: any) {
    ...
    for (const req of companyType.requirements) {
      const currentAmount = bot.resources.find((r) => r.resourceId === req.resourceId)?.amount || 0;
    ...
    const marketResources = await this.resourcesService.getMarketResources(bot.gameSessionId);
  
    for (const [resourceId, needed] of neededResources) {
      try {
        const marketResource = marketResources.find((r) => r.resource.id === resourceId);
        if (!marketResource) continue;
  
        const cost = marketResource.resource.resourCecost;
        const canAfford = Math.min(Math.floor(bot.playerBalance / cost), needed, marketResource.quantity);
  
        if (canAfford > 0) {
          await this.resourcesService.buyFromMarket(bot.id, resourceId, bot.gameSessionId);
          this.logger.log(`Бот ${bot.id} купил ресурс ${resourceId}`);
        }
      } catch (error) {
        this.logger.error(`Бот ${bot.id} не смог купить ресурс: ${error.message}`);
      }
    }
  }
  ...
  private async sellUnusedResources(bot: any) {
    ...
    if (botState?.targetCompanyTypeId) {
      const targetCompanyType = allCompanyTypes.find((t) => t.id === botState.targetCompanyTypeId);
      targetCompanyType?.requirements.forEach((req) => usedResources.add(req.resourceId));
    }
  
    for (const resource of bot.resources) {
      if (!usedResources.has(resource.resourceId) && resource.amount > 0) {
        try {
          await this.resourcesService.sellToMarket(bot.id, resource.resourceId, bot.gameSessionId);
          this.logger.log(`Бот ${bot.id} продал ненужный ресурс ${resource.resourceId}`);
        } catch (error) {
          this.logger.error(`Бот ${bot.id} не смог продать ресурс ${error.message}`);
        }
      }
    }
  }
  ...
  private async handleSharesTrading(bot: any) {
    try {
      const news = await this.newsService.getActiveNewsForSession(bot.gameSessionId);
      const playerShares = await this.sharesService.getPlayerShares(bot.id);
      const allShares = await this.sharesService.getAvailableShares(bot.gameSessionId, bot.id);
      ...
      for (const owned of playerShares) {
        ...
        const shouldSell = (effect !== undefined && effect < 0) || (sharePrice < avgBuyPrice && noNewsSellChance);
  
        if (shouldSell) {
          const quantity = owned.quantity;
          if (quantity > 0) {
            await this.sharesService.sellShares(bot.id, sharesId, quantity);
            this.logger.log(`Бот ${bot.id} продал ${quantity} акций компании ${company.id}`);
            return;
          }
        }
      }
      ...
      const affordableShares = allShares.filter((s) => s.costShares && s.costShares <= bot.playerBalance);
      ...
      const finalSharesList = filteredShares.length > 0
        ? filteredShares
        : noNewsBuyChance
          ? affordableShares
          : [];
  
      const randomShare = finalSharesList[Math.floor(Math.random() * finalSharesList.length)];
      ...
      if (randomShare) {
        ...
        if (maxQuantity > 0) {
          ...
          this.logger.log(`Бот ${bot.id} купил ${quantity} акций компании ${randomShare.company.id}`);
        }
      }
    } catch (error) {
      this.logger.error(`Бот ${bot.id} не смог обработать торговлю акциями: ${error.message}`);
    }
  }
